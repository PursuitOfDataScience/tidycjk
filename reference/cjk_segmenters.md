...
atomic element is coerced with
[`as.character()`](https://rdrr.io/r/base/character.html), so an engine
returning integers or a factor works; a list element is refused, because
...
as the single token `"c(1, 2)"` – R code spelled out as data.
