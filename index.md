...
| **Normalise** | [`to_halfwidth()`](https://pursuitofdatascience.github.io/tidycjk/reference/to_halfwidth.md) · [`to_fullwidth()`](https://pursuitofdatascience.github.io/tidycjk/reference/to_halfwidth.md) · [`cjk_normalize()`](https://pursuitofdatascience.github.io/tidycjk/reference/cjk_normalize.md) |
...
| **NLP prep** | [`cjk_sentences()`](https://pursuitofdatascience.github.io/tidycjk/reference/cjk_sentences.md) · [`cjk_ngrams()`](https://pursuitofdatascience.github.io/tidycjk/reference/cjk_ngrams.md) · [`cjk_strip_punct()`](https://pursuitofdatascience.github.io/tidycjk/reference/cjk_strip_punct.md) |
...
| **Reference** | [`cjk_blocks()`](https://pursuitofdatascience.github.io/tidycjk/reference/cjk_blocks.md) |
...
## Normalise: surgical or blunt

![to_halfwidth narrows width forms and leaves everything else
alone.](reference/figures/fig-normalize.png)

Two tools, and the choice is deliberate.
[`to_halfwidth()`](https://pursuitofdatascience.github.io/tidycjk/reference/to_halfwidth.md)
changes width and nothing else — where the usual `NFKC` advice would
also rewrite ligatures, Roman numerals and circled numbers.
...
is the blunt instrument, for when matching rather than display is the
point. Plain `"nfc"` is not a no-op on Han either: it rewrites 460 of
the 472 CJK compatibility ideographs.
...
`gsub("[[:punct:]]", "", x)` is resolved by the C library, so it removes
nothing under `LC_ALL=C`; `perl = TRUE` removes nothing from CJK in any
locale.
[`cjk_strip_punct()`](https://pursuitofdatascience.github.io/tidycjk/reference/cjk_strip_punct.md)
tests the Unicode category instead.
...
# and it keeps ー, a modifier letter rather than a dash
...
Sorting is its own trap. [`sort()`](https://rdrr.io/r/base/sort.html)
reads `LC_COLLATE`, so the order it gives Han depends on the machine:
code point order under `C`, pinyin under `zh_CN.utf8`, something else
again under `ja_JP.utf8`. Naming a collation is what makes the answer
the same everywhere:
